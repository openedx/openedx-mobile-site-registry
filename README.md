...
Put your production secrets/config (`SECRET_KEY`, `SMTP_*`, `WEBHOOK_URL`, …) in a
`.env` file next to the compose file, and run the container behind your own
reverse proxy (Traefik, Caddy, nginx, …) terminating TLS for your domain. SQLite
data persists in `./data` — mount it as a volume in production.
